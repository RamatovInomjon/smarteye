...
PopUp::PopUp(bool isError, QWidget *parent) : QWidget(parent) {
  ...
  animation.setPropertyName(
      "popupOpacity"); // Устанавилваем анимируемое свойство
  ...
  label.setAlignment(Qt::AlignHCenter |
                     Qt::AlignVCenter); // Устанавилваем по центру
  // И настраиваем стили
...
void PopUp::paintEvent(QPaintEvent *event) {
  ...
  if (isError)
    painter.setBrush(QBrush(QColor(180, 0, 0, 220)));
  else
...
void PopUp::show() {
  ...
  animation.setStartValue(
      0.0); // Стартовое значение будет 0 (полностью прозрачный виджет)
  ...
  setGeometry(w - 40 - width(), h - 40 - height(), width(), height());
  ...
  timer->start(3000); // А также стартуем таймер, который запустит скрытие
                      // уведомления через 3 секунды
}

void PopUp::hideAnimation() {
  timer->stop(); // Останавлвиваем таймер
  animation.setDuration(2500); // Настравиваем длительность анимации
  animation.setStartValue(
      1.0); // Стартовое значение будет 1 (полностью непрозрачный виджет)
