...
static const QString kEncryptorString =
    QString("texnouz001"); // here is key to crypt/decrypt QString;
...
public:
  static QString cryptString(QString toCrypt) {
    ...
    for (int i = 0; i < toCrypt.length(); i++) {
      // resultString.append(
      //          QString(QChar(toCrypt[i]).unicode() ^
      //                  QChar(kEncryptorString[keyIndex(i)]).unicode()));
    }
    return resultString;
  }
  static QString decryptString(QString toDecrypt) {
    ...
    for (int i = 0; i < toDecrypt.length(); i++) {
      // resultString.append(
      //          QString(QChar(toDecrypt[i]).unicode() ^
      //                  QChar(kEncryptorString[keyIndex(i)]).unicode()));
    }
    return resultString;
  }
